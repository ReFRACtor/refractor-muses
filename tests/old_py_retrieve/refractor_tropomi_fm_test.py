...
def test_refractor_fm_muses_py(
    ...
    assert (
        np.abs(
            o_measured_radiance_tropomi["measured_radiance_field"]
            - o_measured_radiance_tropomi2["measured_radiance_field"]
        ).max()
        < 1e-15
    )
    assert (
        np.abs(
            o_measured_radiance_tropomi["measured_nesr"]
            - o_measured_radiance_tropomi2["measured_nesr"]
        ).max()
        < 1e-15
    )
...
def test_refractor_joint_fm_refractor(
    ...
    if True:
        ...
        sns.relplot(
            ...
            hue="Type",
            kind="line",
        ).set(title=f"CrIS-TROPOMI Strategy step {step_number}, iteration {iteration}")
        plt.tight_layout()
...
def test_refractor_detailed_fm_refractor(
    ...
    if True:
        ...
        sns.relplot(
            ...
            hue="Type",
            kind="line",
        ).set(title=f"TROPOMI Strategy step {step_number}, iteration {iteration}")
        plt.tight_layout()
        plt.savefig("plot1.png", dpi=300)
        d2 = pd.DataFrame(
            {"Wavelength (nm)": sd, "Difference Radiance": o_radiance - o_radiance2}
...
def test_jac_fd(
    ...
    print(
        f"Finite difference jacobian for index {index} ({nm[index]}) using delta {delta[index]}:"
    )
    ...
    if True:
        ...
        sns.relplot(
            ...
            hue="Type",
            kind="line",
        ).set(title=f"Index {index} ({nm[index]}) using delta {delta[index]}")
        plt.tight_layout()
        plt.savefig("plot1.png", dpi=300)
        d2 = pd.DataFrame({"Wavelength (nm)": sd, "Difference Jac": jfd - jcalc})
        sns.relplot(data=d2, x="Wavelength (nm)", y="Difference Jac", kind="line").set(
            title=f"Index {index} ({nm[index]}) using delta {delta[index]}"
        )
        plt.tight_layout()
        plt.savefig("plot2.png", dpi=300)
