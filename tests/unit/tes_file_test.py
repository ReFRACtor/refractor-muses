...
def test_tes_file(osp_dir, omi_test_in_dir):
    fname = omi_test_in_dir / "Table.asc"
    tfile = TesFile.create(fname, None)
    # Check that second call returns same object
    tfile_second = TesFile.create(fname, None)
    tfile_third = TesFile.create(fname, None)
    assert tfile is tfile_second
    assert tfile is tfile_third
    # Verify against the mpy version
    tfile2 = TesFile(fname, None, use_mpy=True)
    assert dict(tfile) == dict(tfile2)
    assert tfile.table.equals(tfile2.table)

    # Try another file that has no table
    fname = osp_dir / "Strategy_Tables/ops/OSP-OMI-v2/Species-66/OMICLOUDFRACTION.asc"
    tfile = TesFile.create(fname, None)
    tfile2 = TesFile(fname, None, use_mpy=True)
    assert dict(tfile) == dict(tfile2)
    ...
    fname = osp_dir / "Strategy_Tables/ops/OSP-OMI-v2/Cloud/CloudParameters.asc"
    tfile = TesFile.create(fname, None)
    tfile2 = TesFile(fname, None, use_mpy=True)
    assert dict(tfile) == dict(tfile2)
    assert tfile.table.equals(tfile2.table)
    ...
    fname = osp_dir / "Strategy_Tables/ops/OSP-OMI-v2/Species-66/CO2_stepco2_1.asc"
    tfile = TesFile.create(fname, None)
    tfile2 = TesFile(fname, None, use_mpy=True)
    assert dict(tfile) == dict(tfile2)
    assert tfile.table is None
