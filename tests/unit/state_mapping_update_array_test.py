...
def test_state_mapping_update_array():
    ...
    npt.assert_allclose(
        smap.mapped_state(update_coeff).jacobian, [[1, 0], [0, 0], [0, 0], [0, 1]]
    )
    npt.assert_allclose(
        smap.retrieval_state(smap.mapped_state(update_coeff)).value, [11, 14]
    )
    npt.assert_allclose(
        smap.retrieval_state(smap.mapped_state(update_coeff)).jacobian, [[1, 0], [0, 1]]
    )
