...
class OmiLevel1Simulation(OmiLevel1RadianceFile):
    ...
    def __init__(self, l1b_filename, simulation_filename, along_track_index, across_track_indexes):
    ...
    def sample_grid(self, chan_index: int) -> rf.SpectralDomain:
        ...
        with netCDF4.Dataset(self.simulation_filename, "r") as sim_file:
            grid_var = sim_file[f"/Retrieval/Step_1/Initial/Spectrum/Channel_{chan_index + 1}/Convolved/grid"]
            return rf.SpectralDomain(grid_var[:], rf.Unit("nm"))

    def radiance(self, chan_index: int) -> rf.SpectralRange:

        with netCDF4.Dataset(self.simulation_filename, "r") as sim_file:
            rad_var = sim_file[f"/Retrieval/Step_1/Initial/Spectrum/Channel_{chan_index + 1}/Convolved/radiance"]
...
class OmiSimulatedRadianceCreator(creator.base.Creator):
    ...
    def create(self, **kwargs):
        ...
        return OmiLevel1Simulation(self.input_filename(), self.simulation_filename(), self.along_track_index(), self.across_track_indexes())
...
def retrieval_config(**kwargs):
    ...
    config_def['input']['l1b']['creator'] = creator.modifier.singleton(OmiSimulatedRadianceCreator)
