...
class OmiIlsTable(creator.instrument.IlsTable):
    ...
    def create(self, **kwargs):
        ...
        for chan_idx in range(self.num_channels()):
            ...
            ils_filename = os.path.join(self.ils_path(), chan_name, "OMI_ILS_NORMAL_{}_{:02d}.h5".format(chan_name, across_track_indexes[chan_idx]))
            ...
            chan_center_wn = np.flip(rf.ArrayWithUnit(chan_center_wl, "nm").convert_wave("cm^-1").value)
            chan_freq_wn = np.flip(rf.ArrayWithUnit(chan_freq_wl, "nm").convert_wave("cm^-1").value).transpose()
            ...
            for center_idx in range(chan_center_wn.shape[0]):
                delta_lambda_wn[center_idx, :] = chan_freq_wn[center_idx, :] - chan_center_wn[center_idx]
...
def base_config_definition(micro_windows=None, retrieval_elements=None, solver_parameters=None, covariance_storage=None, **kwargs):
    ...
    if micro_windows is None:
        micro_windows = np.array([[267, 307], # UV-1
                                  [309, 332]]) # UV-2
        micro_windows = rf.ArrayWithUnit(micro_windows, "nm")
        
    config_def = {
        'creator': creator.base.SaveToCommon,
        'order': ['solar_model', 'input', 'common', 'scenario', 'spec_win', 'spectrum_sampling', 'instrument', 'atmosphere', 'radiative_transfer', 'forward_model', 'retrieval'],
        ...
        'atmosphere': {
            ...
            'absorber': {
                ...
                'default_gas_definition': {
                    ...
                    'cross_section': {
                        ...
                        'filename': lambda gas_name=None: rf.cross_section_filenames[gas_name],
                        'conversion_factor': lambda gas_name=None: rf.cross_section_file_conversion_factors.get(gas_name, 1.0),
                    },
                }, 
            },
            'relative_humidity': {
            ...
            'ground': {
                ...
                'lambertian': {
                    ...
                    'which_retrieved': np.array([[True, False], [True, True]], dtype=bool),
                },
            },
        },
        'radiative_transfer': {
        ...
        'retrieval': {
            ...
            'covariance': {
                ...
                'values': {
                    ...
                    'ground/lambertian': np.identity(num_channels*2) * np.array([2.5e-3, 1.0e-4]*num_channels),
