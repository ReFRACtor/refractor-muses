...
class OmiSimConfig(rf.MusesUipSimConfig):
    ...
    def __init__(self, muses_uip_file, config_def, atm_gas_list=None):
        super().__init__(
            muses_uip_file, config_def, num_channels, atm_gas_list=atm_gas_list
        )
    ...
    def configure_scenario(self):
        ...
        self.config_def["scenario"]["across_track_indexes"] = self.uip_omi[
            "omi_obs_table"
        ][0]["xtrack"][0]

        self.config_def["solar_model"]["across_track_indexes"] = self.uip_omi[
            "omi_obs_table"
        ][0]["xtrack"][0]
    ...
    def configure_sample_grid(self):
        ...
        filt_loc = np.array(
            [val.decode("UTF-8") for val in self.uip_omi["frequencyfilterlist"][0]]
        )
        ...
        for band_name in channel_names:
            sample_grid.append(
                rf.SpectralDomain(
                    all_freq[np.where(filt_loc == band_name)], rf.Unit(self.grid_units)
                )
            )
