...
def get_input(name, required=True):
    if not name in os.environ:
        if required:
            raise Exception(f"The environment variable {name} is required to process this configuration")
        else:
            return None
    else:
...
def retrieval_run_config(**kwargs):
    ...
    if not len(across_track_indexes) == 2:
        raise Exception("Expected 2 integers for across track indexes seperated by commas or spaces")

    return retrieval_muses_config_definition(l1b_filename, along_track_index, across_track_indexes, osp_dir, irradiance_file=irr_filename, gmao_dir=gmao_dir, **kwargs)
