...
def features_l1b(
    ...
    temp_prior: np.ndarray | None = None,
    h2o_prior: np.ndarray | None = None,
    ...
    if prior is not None:
        ...
        apriori: Any = np.reshape(prior, (n_obs, len(prior[0, 0, 0, 0, :])))
    else:
        apriori = None
    if temp_prior is not None:
        # This is not properly implemented yet
        temp_apriori: Any = np.reshape(
            temp_prior, (n_obs, len(temp_prior[0, 0, 0, 0, :]))
        )
    else:
        temp_apriori = None
    if h2o_prior is not None:
        # This is not properly implemented yet
        h2o_apriori: Any = np.reshape(h2o_prior, (n_obs, len(h2o_prior[0, 0, 0, 0, :])))
    else:
        h2o_apriori = None
...
class MlPredictionClass:
    ...
    def __init__(
        self,
        var1: str | np.ndarray | None,
        var2: np.ndarray | None,
        var3: np.ndarray | None,
        var4: np.ndarray | None,
        var5: np.ndarray | None,
        var6: np.ndarray | None,
        var7: np.ndarray | None,
        var8: np.ndarray | None,
        var9: np.ndarray | None,
        var10: float | np.ndarray | None,
    ) -> None:
        if var1 == "regression":
            self.labels = var2
...
def prediction(
    mdl: Any = None,
    mdl_api: str = "sequential",
    mdl_type: str = "regression",
    ...
    features: Any = None,
    labels: Any = None,
    enc_features: bool = False,
    enc_features_path: str = "",
    enc_features_prefix: str = "",
    enc_features_suffix: str = "",
    enc_labels: bool = False,
    enc_labels_path: str = "",
    enc_labels_prefix: str = "",
    enc_labels_suffix: str = "",
    features_transform: str = "standardize",
    labels_transform: str = "standardize",
    standardize_features: bool = True,
    renorm_labels: bool = False,
    renorm_pred: bool = True,
    labels_log_scale: bool = False,
    labels_negexp_scale: bool = False,
    batch_size_in: int = 32,
    scale_fac: float = 1.0,
    scale_fac_conv: float = 1.0,
    ensemble: bool = False,
    prob_labels: bool = False,
    n_pred: int = 10,
    ...
    # 1) Some housekeeping
    ##########################################################################
    file_in_weights = Path(path) / f"{prefix}_weights_{suffix}.keras"
    file_in_scale = Path(path) / f"{prefix}_scaling_{suffix}.h5"
    if extra_suffix != "":
        file_out_evaluate = (
            Path(path) / f"{prefix}_evaluation_{suffix}{extra_suffix}.h5"
        )
    else:
        file_out_evaluate = Path(path) / f"{prefix}_evaluation_{suffix}.h5"
    ...
    if mdl_type == "regression":
        if evaluate is True:
            ...
            for i in range(0, n_labels):
                ...
                corr_coeff[i] = pearsonr(  # type:ignore[call-overload]
