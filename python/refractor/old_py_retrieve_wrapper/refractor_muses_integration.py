...
class RefractorMusesIntegration(mpy.ReplaceFunctionObject):
    ...
    def run_retrieval_zero_iterations(
        self,
        i_stateInfo,
        i_tableStruct,
        i_windows,
        ...
        if isinstance(i_retrievalInfo, RetrievalInfo):
            ...
            jacobian_speciesNames = i_retrievalInfo.species[
                0 : i_retrievalInfo.n_species
            ]
            jacobian_speciesList = i_retrievalInfo.speciesListFM[
                0 : i_retrievalInfo.n_totalParametersFM
            ]
        ...
        if jacobian_speciesList[0] != "":
            if isinstance(i_retrievalInfo, RetrievalInfo):
                ...
                xret = i_retrievalInfo.constraintVector[
                    0 : i_retrievalInfo.n_totalParameters
                ]
        else:
            jacobianOut = 0
            xret = 0
        o_retrievalResults = {
            ...
            "stopCode": -1,
            "stopCriteria": np.zeros(shape=(1, 3), dtype=int),
            "resdiag": np.zeros(shape=(1, 5), dtype=int),
            "xret": xret,
    ...
    def run_retrieval(
        ...
        if self.save_debug_data:
            ...
            pickle.dump(
                sve, open(f"run_retrieval_step_{i_tableStruct['step']}.pkl", "wb")
            )
        ...
        maxIter = int(
            mpy.table_get_entry(
                i_tableStruct, i_tableStruct["step"], "maxNumIterations"
            )
        )
        if maxIter == 0:
        ...
        if len(tweaked_array_radiance.shape) == 1:
            tweaked_array_radiance = np.reshape(
                tweaked_array_radiance, (1, tweaked_array_radiance.shape[0])
            )
        if len(tweaked_array_nesr.shape) == 1:
            tweaked_array_nesr = np.reshape(
                tweaked_array_nesr, (1, tweaked_array_nesr.shape[0])
            )
        ...
        ret_info = {
            ...
            "const_vec": xa,
            "minimumList": i_retrievalInfo.minimumList[
                0 : i_retrievalInfo.n_totalParameters
            ],
            "maximumList": i_retrievalInfo.maximumList[
                0 : i_retrievalInfo.n_totalParameters
            ],
            "maximumChangeList": i_retrievalInfo.maximumChangeList[
                0 : i_retrievalInfo.n_totalParameters
            ],
        }

        # Various thresholds from the input table
        ConvTolerance_CostThresh = np.float32(
            mpy.table_get_pref(i_tableStruct, "ConvTolerance_CostThresh")
        )
        ConvTolerance_pThresh = np.float32(
            mpy.table_get_pref(i_tableStruct, "ConvTolerance_pThresh")
        )
        ConvTolerance_JacThresh = np.float32(
            mpy.table_get_pref(i_tableStruct, "ConvTolerance_JacThresh")
        )
        Chi2Tolerance = 2.0 / meas_err.size  # theoretical value for tolerance
        retrievalType = mpy.table_get_entry(
            i_tableStruct, i_tableStruct["step"], "retrievalType"
        ).lower()
        ...
        delta_str = mpy.table_get_pref(
            i_tableStruct, "LMDelta"
        )  # 100 // original LM step size
        ...
        cfunc = self.cost_function_creator.cost_function_from_uip(
            rf_uip, self.obslist, ret_info, **self.kwargs
        )

        outputDir = i_tableStruct["dirStep"]
        dir_tokens = outputDir.split(
            "/"
        )  # e.g. ..//00_AIRS_LAND-20130629_189_000_09/Step01_TATM, H2O, HDO, N2O, CH4, TSUR, CLOUDEXT, EMIS/
        if dir_tokens[-1] == "":
        ...
        with register_replacement_function_in_block("residual_fm_jacobian", cfunc):
            (
                xret,
        ...
        if isinstance(i_retrievalInfo, RetrievalInfo):
            ...
            speciesFM = i_retrievalInfo.speciesListFM[
                0 : i_retrievalInfo.n_totalParametersFM
            ]
        jacobianOut2 = mpy.jacobian_data(
            jacobian_fm, detectors, i_radianceInfo["frequency"], speciesFM
        )
    ...
    def run_forward_model(
        ...
        if self.save_debug_data:
            ...
            sve.capture_directory.save_directory(".", vlidort_input=None)
            pickle.dump(
                sve, open(f"run_forward_model_step_{i_table['step']}.pkl", "wb")
            )
        rf_uip = RefractorUip.create_uip(
            i_stateInfo,
            i_table,
        ...
        # for CostFunction.
        cfunc = self.cost_function_creator.cost_function_from_uip(
            rf_uip, self.obslist, None, include_bad_sample=True, **self.kwargs
        )
        (o_radiance, jac_fm, _, _, _) = cfunc.fm_wrapper(rf_uip.uip, None, {})
        o_jacobian = mpy.jacobian_data(
            jac_fm,
            o_radiance["detectors"],
