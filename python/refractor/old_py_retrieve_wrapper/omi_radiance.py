...
class OmiRadiancePyRetrieve(OmiRadiance):
    ...
    def __init__(self, rf_uip, include_bad_sample=False, skip_jac=False):
        ...
        coeff = [self.rf_uip.omi_params["nradwav_uv1"],
        ...
        which_retrieved = ["OMINRADWAVUV1" in self.rf_uip.state_vector_params("OMI"),
        ...
        self.mw = [slice(0, self.rf_uip.nfreq_mw(0, "OMI")),
    ...
    def spectral_domain(self, sensor_index, inc_bad_sample=False):
        ...
        if(inc_bad_sample or self._force_include_bad_sample):
            gmask[:] = True
        sd = rf.SpectralDomain(o_measured_radiance_omi["wavelength"][self.mw[sensor_index]][gmask], rf.Unit("nm"))
        return sd
    ...
    def bad_sample_mask_full(self, sensor_index):
        ...
        o_measured_radiance_omi = self.rf_uip.measured_radiance("OMI", sensor_index,
                                                                full_freq=True)
    ...
    def radiance(self, sensor_index, skip_jacobian=False,
        ...
        if(inc_bad_sample or self._force_include_bad_sample):
            gmask[:] = True
        sd = rf.SpectralDomain(o_measured_radiance_omi["wavelength"][lmw][gmask], rf.Unit("nm"))
        if(self.skip_jac or self.mapped_state.is_constant):
            jac = np.empty((len(o_measured_radiance_omi["measured_radiance_field"][lmw][gmask]),0))
        else:
            jac = (np.outer(-o_measured_radiance_omi['normwav_jac'][lmw][gmask],
                            self.mapped_state.jacobian[0+sensor_index,:]) +
                   np.outer(-o_measured_radiance_omi['odwav_jac'][lmw][gmask],
                            self.mapped_state.jacobian[2+sensor_index,:]) +
                   np.outer(-o_measured_radiance_omi['odwav_slope_jac'][lmw][gmask],
                            self.mapped_state.jacobian[4+sensor_index,:]))
        sr = rf.SpectralRange(rf.ArrayAd_double_1(o_measured_radiance_omi["measured_radiance_field"][lmw][gmask], jac), rf.Unit("sr^-1"), o_measured_radiance_omi["measured_nesr"][lmw][gmask])
