...
class RetrievalInfoOld:
    ...
    def __init__(self, strategy_table: "StrategyTable", state_info: "StateInfo"):
        (self.retrieval_dict, _, _) = self.init_data(
            strategy_table.strategy_table_dict, state_info.state_info_dict
        )
    ...
    def species_initial(self, spcname, FM_Flag=True):
        return mpy.get_vector(
            self.initialGuessList, self.retrieval_info_obj, spcname, FM_Flag, True
        )

    def species_constraint(self, spcname, FM_Flag=True):
        return mpy.get_vector(
            self.constraintVector, self.retrieval_info_obj, spcname, FM_Flag, True
        )
    ...
    @property
    def species_list_fm(self):
        return self.retrieval_dict["speciesListFM"][
            0 : self.retrieval_dict["n_totalParametersFM"]
        ]
    ...
    def init_data(self, strategy_table: "StrategyTable", state_info: "StateInfo"):
        ...
        o_retrievalInfo = {
            ...
            "altitudeList": [
                0.0 for x in range(10 * nn + 100)
            ],  # not needed in retrieval system; for archiving
            ...
            "mapToState": np.zeros(
                shape=(nn * 10 + 100, nn * 10 + 100), dtype=np.float64
            ),
            "mapToParameters": np.zeros(
                shape=(nn * 10 + 100, nn * 10 + 100), dtype=np.float64
            ),
        ...
        o_retrievalInfo["type"] = mpy.table_get_entry(
            i_table_struct, step, "retrievalType"
        )
        ...
        if o_errorInitial is None:
            ...
            o_errorInitial = mpy.get_prior_error(
                i_table_struct.errorSpecies,
                i_table_struct.errorMaptype,
        ...
        o_errorCurrent = copy.deepcopy(
            o_errorInitial
        )  # Make a separate copy of o_errorInitial to o_errorCurrent.
        ...
        if (
            retrievalType == "bt" or retrievalType == "forwardmodel"
        ):  # or retrievalType == 'fullfilter': EM NOTE - keep fullfilter in the same bracket as 'default'
            # no retrieval info
            ...
            result = mpy.table_get_unpacked_entry(
                i_table_struct, step, "retrievalElements"
            )
            ...
            for ii in range(num_species):
                ...
                speciesInformationFilename = (
                    mpy.table_get_pref(i_table_struct, "speciesDirectory")
                    + os.path.sep
                    + species_name
                    + retrievalTypeStr
                    + ".asc"
                ...
                speciesInformationFile = mpy.ObjectView(
                    speciesInformationFile
                )  # It is now an object.
                ...
                if "OMI" in species_name and "TROPOMI" not in species_name:
                    ...
                    if have_true:
                        trueParameterList = stateInfo.true["tropomi"][
                            actual_tropomi_key
                        ]
                    constraintVector = stateInfo.constraint["tropomi"][
                        actual_tropomi_key
                    ]
                    constraintVectorFM = stateInfo.constraint["tropomi"][
                        actual_tropomi_key
                    ]
                ...
                # IDL AT_LINE 240 Get_Species_Information:
                    ...
                    if have_true:
                        trueParameterListFM = (
                            stateInfo.true["nir"]["albpl"].reshape(nn) * mult
                        )
                    constraintVectorFM = (
                        stateInfo.constraint["nir"]["albpl"].reshape(nn) * mult
                    )
                    ...
                    if (mapType == "linearpca") or (mapType == "logpca"):
                        ...
                        (constraintStruct, constraintPressure) = mpy.constraint_read(
                            filename
                        )
                        constraintMatrix = mpy.constraint_get(constraintStruct)
                        constraintPressure = mpy.constraint_get_pressures(
                            constraintStruct
                        )
                        ...
                        if mapType == "linearpca":
                            ...
                            initialGuessListFM = np.exp(
                                np.log(constraintVectorFM)
                                + mapToState @ initialGuessList
                            )
                            if have_true:
                                trueParameterList = np.transpose(mapToParameters) @ (
                                    np.log(trueParameterListFM)
                                    - np.log(constraintVectorFM)
                                )
                    # end part of elif (mapType == 'linearpca') or (mapType == 'logpca'):
                        ...
                        if mm == nn:
                            ...
                            ind1 = np.where(
                                (pressureList > 1.0) * (pressureList < 2.0)
                            )[0]
                            ind2 = np.where(
                                (pressureListFM > 1.0) * (pressureListFM < 2.0)
                            )[0]
                            if len(ind2) > 0:
                                pressureList[min(ind1)] = np.min(pressureListFM[ind2])
                                pressureList[max(ind1)] = np.max(pressureListFM[ind2])
                            ...
                            ind1 = np.where(
                                (pressureList > 2.0) * (pressureList < 2.2)
                            )[0]
                            ind2 = np.where(
                                (pressureListFM > 2.0) * (pressureListFM < 2.2)
                            )[0]
                            if len(ind2) > 0:
                                pressureList[min(ind1)] = np.min(pressureListFM[ind2])
                                pressureList[max(ind1)] = np.max(pressureListFM[ind2])
                        ...
                        for iq in range(len(pressureList)):
                            ...
                            ind = np.where(
                                abs(pressureList[iq] - pressureListFM) == xx
                            )[0][0]
                        ...
                        constraintVector = (
                            maps["toPars"].transpose() @ constraintVectorFM
                        )
                        initialGuessList = (
                            maps["toPars"].transpose() @ initialGuessListFM
                        )
                        if have_true:
                            trueParameterList = (
                                maps["toPars"].transpose() @ trueParameterListFM
                            )
                    ...
                    if len(value) > 1:
                        speciesInformationFile.sSubaDiagonalValues = np.array(value)[
                            myinds
                        ]
                    ...
                    val = np.array(
                        (speciesInformationFile.sSubaDiagonalValues).split(",")
                    )
                    constraintMatrix = np.identity(mm)
                    for jj in range(mm):
                    ...
                    if "NIRAER" in species_name:
                        ...
                        sSubaDiagonalValues = f.get_preference(
                            "sSubaDiagonalValues"
                        ).split(",")
                        ...
                        for ik in range(stateInfo.current["nir"]["naer"]):
                            if (str(stateInfo.current["nir"]["aertype"][ik].dtype))[
                                1
                            ] == "S":
                                ind = np.where(
                                ...
                                ind = np.where(
                                    stateInfo.current["nir"]["aertype"][ik]
                                    == np.array(types)
                                )[0]
                        ...
                        speciesInformationFile.minimum = ",".join(
                            np.array(minimum)[myinds]
                        )
                        speciesInformationFile.maximum = ",".join(
                            np.array(maximum)[myinds]
                        )
                    ...
                    if (
                        ...
                        npoly = int(
                            len((speciesInformationFile.sSubaDiagonalValues).split(","))
                            / 3
                        )
                        ...
                        initialGuessList = stateInfo.current["nir"][mykey].reshape(
                            npar * nband
                        )
                    ...
                    if (
                        ...
                        pressureListFM = (
                            stateInfo.current["nir"]["albplwave"]
                        ).reshape(len(stateInfo.current["nir"]["albplwave"]))
                    ...
                    if species_name == "EMIS":
                        ...
                        averageFlag = False
                        maps = mpy.make_maps(
                            stateInfo.emisPars["frequency"][0:nn],
                            ind,
                            linearFlag,
                            averageFlag,
                        )
                        mapToState = maps["toState"]  # SPECIES_NAME 'EMIS'
                        mapToParameters = maps[
                            "toPars"
                        ]  # mapToParameters.shape (121, 2)
                    ...
                    if species_name == "CLOUDEXT":
                        ...
                        filename = mpy.table_get_pref(
                            i_table_struct, "CloudParameterFilename"
                        )
                        ...
                        maps = mpy.make_maps(
                            stateInfo.cloudPars["frequency"][0:nn],
                            ind,
                            linearFlag,
                            averageFlag,
                        )
                        ...
                        initialGuessListFM = np.log(
                            stateInfo.current["cloudEffExt"][0, 0:nn]
                        )
                        if have_true:
                            trueParameterListFM = np.log(
                                stateInfo.true["cloudEffExt"][0, 0:nn]
                            )
                    ...
                    if species_name == "CALSCALE":
                        ...
                        altitudeListFM = (
                            stateInfo.calibrationPars["frequency"][0:nn] / 100.0
                        )
                        constraintVectorFM = stateInfo.constraint["calibrationScale"][
                            0:nn
                        ]
                        initialGuessListFM = stateInfo.current["calibrationScale"][0:nn]
                        if have_true:
                            trueParameterListFM = stateInfo.true["calibrationScale"][
                                0:nn
                            ]
                        assert False
                    ...
                    if species_name == "CALOFFSET":
                        ...
                        constraintVectorFM = stateInfo.constraint["calibrationOffset"][
                            0:nn
                        ]
                        initialGuessListFM = stateInfo.current["calibrationOffset"][
                            0:nn
                        ]
                        if have_true:
                            trueParameterListFM = stateInfo.true["calibrationOffset"][
                                0:nn
                            ]
                        assert False
                    ...
                    if constraintType.lower() == "tikhonov":
                        ...
                        for ll in range(0, mm - 2):
                            if pressureList[ll + 1] - pressureList[ll] < 30:
                                constraintMatrix[ll, ll] = (
                                    constraintMatrix[ll, ll] + myError
                                )
                ...
                # AT_LINE 489 Get_Species_Information.pro
                    ...
                    # set pars for retrieval
                    constraintVector = stateInfo.constraint["tes"][
                        "boresightNadirRadians"
                    ]
                    constraintVectorFM = stateInfo.constraint["tes"][
                        "boresightNadirRadians"
                    ]
                    initialGuessList = stateInfo.current["tes"]["boresightNadirRadians"]
                    initialGuessListFM = stateInfo.current["tes"][
                        "boresightNadirRadians"
                    ]
                    if have_true:
                        trueParameterList = stateInfo.true["tes"][
                            "boresightNadirRadians"
                        ]
                        trueParameterListFM = stateInfo.true["tes"][
                            "boresightNadirRadians"
                        ]
                    ...
                    if have_true:
                        ...
                        if stateInfo.true["num_clouds"] == 2:
                            ...
                            if np.sum(c1) > np.sum(c2):
                                trueParameterList = math.log(
                                    stateInfo.true["PCLOUD"][1]
                                )
                ...
                # end part of elif (species_name == 'PSUR'):
                    ...
                    altitudeList = stateInfo.current["heightKm"][
                        retrievalParameters - 1
                    ]
                ...
                # end part of elif (mapType == 'linearscale') or (mapType == 'logscale'):
                    ...
                    constraintVectorFM = stateInfo.constraint["values"][ind, :].reshape(
                        nn
                    )
                    if have_true:
                    ...
                    if mapType == "linearpca":
                        ...
                        if have_true:
                            trueParameterList = np.transpose(mapToParameters) @ (
                                np.log(trueState) - np.log(constraintVectorFM)
                            )
                            trueParameterListFM = np.copy(trueState)
                # end part of elif (mapType == 'linearpca') or (mapType == 'logpca'):
                    ...
                    if (mapType == "linear") or (mapType == "log"):
                        ...
                        if any_values_greater_than_size:
                            o_cleaned_retrievalParameters = (
                                utilLevels.RemoveIndicesTooBig(
                                    retrievalParameters, pressure, function_name
                                )
                            )
                        ...
                        maps = mpy.make_maps(
                            stateInfo.current["pressure"], retrievalParameters
                        )
                        maps = mpy.ObjectView(maps)
                        mapToParameters = maps.toPars
                        mapToState = maps.toState
                    ...
                    if o_retrievalInfo.type == "2" and species_name == "NH3":
                        ...
                        currentGuess = initialGuess * 2 / 3.0
                    ...
                    if mapType.lower() == "log":
                        if have_true:
                            if (
                                mpy.table_get_pref(
                                    i_table_struct, "mapTrueFullStateVector"
                                )
                                == "yes"
                            ):
                                trueStateFM = np.exp(
                        ...
                        if (
                            mpy.table_get_pref(i_table_struct, "mapInitialGuess")
                            == "yes"
                        ):
                        ...
                        if (
                            ...
                            initialGuess = np.matmul(
                                initialGuessFM,
                                np.matmul(mapToParameters, mapToState),
                                initialGuessFM,
                            )  # TRICKY_LOGIC
                            currentGuess = np.matmul(
                                currentGuessFM, np.matmul(mapToParameters, mapToState)
                            )  # TRICKY_LOGIC
                    ...
                    if mapType.lower() == "log":
                        ...
                        constraintVector = np.matmul(
                            constraintVectorFM, mapToParameters
                        )
                        ...
                        if (
                            np.amin(constraintVector) < 0
                            and np.amax(constraintVector) > 0
                        ):
                    ...
                    locs = [
                        (np.where(np.asarray(o_retrievalInfo.species) == "H2O"))[0],
                        (np.where(np.asarray(o_retrievalInfo.species) == "HDO"))[0],
                    ]
                    ...
                    if constraintType == "Scale":
                        if mapType == "linear":
                            initialGuessList = np.mean(
                                initialGuessListFM - constraintVectorFM
                            )
                            initialGuessListFM = constraintVectorFM + initialGuessList
                            if have_true:
                                trueParameterList = np.mean(
                                    trueStateFM - constraintVectorFM
                                )
                                trueParameterListFM = (
                                    constraintVectorFM + trueParameterList
                                )
                        else:
                            initialGuessList = np.mean(
                                initialGuessListFM / constraintVectorFM
                        ...
                        if mapType == "log":
                            ...
                            if have_true:
                                trueParameterList = np.matmul(
                                    np.log(trueState), mapToParameters
                                )
                                trueParameterListFM = np.log(trueStateFM)
                        elif mapType == "linear":
                            initialGuessList = np.matmul(
                                initialGuessFM, mapToParameters
                            )
                            initialGuessListFM = np.copy(initialGuessFM)
                            if have_true:
                                trueParameterList = np.matmul(
                                    trueStateFM, mapToParameters
                                )
                                trueParameterListFM = np.copy(trueStateFM)
                            ...
                            if (
                                np.amin(initialGuessList) < 0
                                and np.max(initialGuessList) > 0
                            ):
                                logger.info(
                            ...
                            if have_true:
                                if (
                                    ...
                                    trueParameterList[ind1] = np.amin(
                                        trueParameterList[ind2]
                                    )
                ...
                o_retrievalInfo.initialGuessListFM[rowFM : rowFM + nn] = (
                    initialGuessListFM[:]
                )
                o_retrievalInfo.constraintVectorListFM[rowFM : rowFM + nn] = (
                    constraintVectorFM[:]
                )

                if have_true:
                    o_retrievalInfo.trueParameterList[row : row + mm] = (
                        trueParameterList[:]
                    )
                    o_retrievalInfo.trueParameterListFM[rowFM : rowFM + nn] = (
                        trueParameterListFM[:]
                    )
                ...
                o_retrievalInfo.mapToState[row : row + mm, rowFM : rowFM + nn] = (
                    mapToState
                )
                o_retrievalInfo.mapToParameters[rowFM : rowFM + nn, row : row + mm] = (
                    mapToParameters
                )
            ...
            sys_tokens = mpy.table_get_unpacked_entry(
                i_table_struct, step, "errorAnalysisInterferents"
            )
            ...
            if sys_tokens[0] != "-" and sys_tokens[0] != "":
                ...
                o_retrievalInfo.speciesSys[0 : o_retrievalInfo.n_speciesSys] = (
                    sys_tokens[:]
                )
                o_retrievalInfo.n_totalParametersSys = len(
                    mpy.constraint_get_species(o_errorInitial, sys_tokens)
                )
            else:
            ...
            if sys_tokens[0] != "-" and sys_tokens[0] != "":
                ...
                for ll in range(0, len(sys_tokens)):
                    ...
                    if len(my_ind) > 0:
                        ...
                        o_retrievalInfo.speciesListSys[my_ind] = (
                            o_retrievalInfo.speciesSys[ll]
                        )
                    else:
        ...
        if locs[0] >= 0 and locs[1] >= 0:
            ...
            for xx in range(2):
                for yy in range(2):
                    ...
                    loc12FM = o_retrievalInfo.parameterEndFM[
                        locs[xx]
                    ]  # Note the spelling of this key 'parameterEndFM'

                    # AT_LINE 887 Get_Species_Information.pro
                    mm = o_retrievalInfo.n_parameters[
                        locs[0]
                    ]  # Note the spelling of this key 'n_parameters'
                    nn = o_retrievalInfo.n_parametersFM[
                        locs[0]
                    ]  # Note the spelling of this key 'n_parametersFM'
                    mapType = "log"
                    mapToParameters = o_retrievalInfo.mapToParameters[
                        loc11FM:loc12FM, loc11:loc12
                    ]  # Note the spelling of this key 'mapToParameters'
