...
class TropomiRadiancePyRetrieve(TropomiRadiance):
    ...
    def __init__(self, rf_uip, skip_jac=False, include_bad_sample=False):
        ...
        coeff = [self.rf_uip.tropomi_params["solarshift_BAND3"],
                 self.rf_uip.tropomi_params["radianceshift_BAND3"],
                 self.rf_uip.tropomi_params["radsqueeze_BAND3"]]
        which_retrieved = ["TROPOMISOLARSHIFTBAND3" in self.rf_uip.state_vector_params("TROPOMI"),
    ...
    def spectral_domain(self, sensor_index, inc_bad_sample=False):
        ...
        if(inc_bad_sample or self._force_include_bad_sample):
            gmask[:] = True
        sd = rf.SpectralDomain(o_measured_radiance_tropomi["wavelength"][gmask], rf.Unit("nm"))
        return sd
    ...
    def radiance(self, sensor_index, skip_jacobian=False, inc_bad_sample=False):
        ...
        if(self.skip_jac or self.mapped_state.is_constant):
            jac = np.empty((len(o_measured_radiance_tropomi["measured_radiance_field"][gmask]),0))
        else:
            mw = [slice(0, self.rf_uip.nfreq_mw(0, "TROPOMI")),
                  slice(self.rf_uip.nfreq_mw(0, "TROPOMI"), None)]
            jac =(np.outer(o_measured_radiance_tropomi['normwav_jac'][mw[0]][gmask],
                           self.mapped_state.jacobian[0,:]) +
                  np.outer(o_measured_radiance_tropomi['odwav_jac'][mw[0]][gmask],
                           self.mapped_state.jacobian[1,:]) +
                  np.outer(o_measured_radiance_tropomi['odwav_slope_jac'][mw[0]][gmask],
                           self.mapped_state.jacobian[2,:]))
                  
        sr = rf.SpectralRange(rf.ArrayAd_double_1(o_measured_radiance_tropomi["measured_radiance_field"][gmask], jac), rf.Unit("sr^-1"), o_measured_radiance_tropomi["measured_nesr"][gmask])
...
class TropomiRadianceRefractor(TropomiRadiance):
    ...
    be correct.
    '''
    def __init__(self, rf_uip, bands, fname="./Input/Radiance_TROPOMI_.pkl",
        ...
        coeff = [self.rf_uip.tropomi_params[f"solarshift_{band}"],
        ...
        self.earth_rad = tropomi['Earth_Radiance']['CalibratedEarthRadiance'][tropomi['Earth_Radiance']['EarthWavelength_Filter'] == band]
        self.sd_data = original_wave[self.rf_uip.freq_index("TROPOMI")]
        self.nesr = tropomi['Earth_Radiance']['EarthRadianceNESR'][tropomi['Earth_Radiance']['EarthWavelength_Filter'] == band]
    ...
    def norm_radiance(self):
        ...
        res = rf.ArrayAd_double_1(len(self.good_freq_index()),
                                  self.mapped_state.number_variable)
        for i,j in enumerate(self.good_freq_index()):
            w = self.orgwav[j] 
    ...
    def norm_rad_nesr(self):
        sol_rad = self.solar_radiance()
        return np.array([self.nesr[j] / sol_rad[i].value for i,j in
                         enumerate(self.good_freq_index())])

    def good_freq_index(self):
        return [int(i) for i in self.rf_uip.freq_index("TROPOMI") if(self.nesr[i] >= 0)]
    ...
    def radiance(self, sensor_index, skip_jacobian=False,
        ...
        if(inc_bad_sample or self._force_include_bad_sample or self.include_bad_sample):
            ...
            nrad_with_bad_data = rf.ArrayAd_double_1(gmask2.shape[0],
                                                     nrad.number_variable)
            j = 0
...
__all__ = ["TropomiRadiance", "TropomiRadiancePyRetrieve",
