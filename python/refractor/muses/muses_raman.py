...
class SurfaceAlbedo(object, metaclass=abc.ABCMeta):
    """MusesRaman needs a surface albedo. This class supplies that,
    however is appropriate for a particular forward model set up. This
    should handle cloud vs clear forward models, similar to
    rf.GroundWithCloudHandling and other classes.

    """

    # TODO Note in generate surface albedo has a jacobian wrt the
    # state vector.  We aren't currently handling propagating that in
    # MusesRaman.
    @abc.abstractmethod
...
class MusesRaman(rf.RamanSiorisEffect):
    ...
    def apply_effect(
        ...
        if sd_min < self.solar_model_sd_min or sd_min > self.solar_model_sd_max:
            raise RuntimeError(
                f"Spectral domain is out of range sd_min = {sd_min} sd_max = {sd_max} raman_sd_min = {self.solar_model_sd_min} raman_sd_max = {self.solar_model_sd_max}"
            )
