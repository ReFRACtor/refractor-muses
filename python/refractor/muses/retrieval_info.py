...
class RetrievalInfo:
    ...
    def add_species(self, species_name, strategy_table, state_info,
        ...
        mm = len(spc.initialGuessList)
        nn = len(spc.initialGuessListFM)
        o_retrievalInfo.pressureList.extend(spc.pressureList)
        o_retrievalInfo.altitudeList.extend(spc.altitudeList)
        o_retrievalInfo.speciesList.extend([species_name] * mm)
        o_retrievalInfo.pressureListFM.append(spc.pressureListFM)
        o_retrievalInfo.altitudeListFM.append(spc.altitudeListFM)
        o_retrievalInfo.speciesListFM.extend([species_name] * nn)
        o_retrievalInfo.constraintVector.append(spc.constraintVector)
        o_retrievalInfo.initialGuessList.append(spc.initialGuessList)
        o_retrievalInfo.initialGuessListFM.append(spc.initialGuessListFM)
        o_retrievalInfo.constraintVectorListFM.append(spc.constraintVectorFM)
        o_retrievalInfo.minimumList.append(spc.minimum)
        o_retrievalInfo.maximumList.append(spc.maximum)
        o_retrievalInfo.maximumChangeList.append(spc.maximum_change)
        o_retrievalInfo.trueParameterList.append(spc.trueParameterList)
        o_retrievalInfo.trueParameterListFM.append(spc.trueParameterListFM)
        o_retrievalInfo.mapToState.append(spc.mapToState)
        o_retrievalInfo.mapToParameters.append(spc.mapToParameters)
        ...
        o_retrievalInfo.mapTypeList.extend([spc.mapType] * mm)
        o_retrievalInfo.mapTypeListFM.extend([spc.mapType] * nn)
        o_retrievalInfo.mapType.append(spc.mapType)
        
        o_retrievalInfo.Constraint[row:row + mm, row:row + mm] = spc.constraintMatrix
