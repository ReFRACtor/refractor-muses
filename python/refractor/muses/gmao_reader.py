...
class GmaoReader:
    ...
    def __init__(
        ...
        if h == 24:
            ...
            day = next_day.day
        gmao_d, f1, f2 = GmaoReader.read_gmao(ifile_hlp, yr, month, day, h)
        lon = gmao_d["lon"]
        lat = gmao_d["lat"]
    ...
    def read_gmao(
        ...
        try:
            pattern_3d = [ template.format(**pattern_params) for template in geos_3d_filename_patterns ]
            fname_3d = next(chain(*[ gmao_fdir.glob(pattern) for pattern in pattern_3d ]))
        except StopIteration:
            raise RuntimeError(f"GMAO 3D file not found from patterns: {pattern_3d} at directory {gmao_fdir}")

        try:
            pattern_2d = [ template.format(**pattern_params) for template in geos_2d_filename_patterns ]
            fname_2d = next(chain(*[ gmao_fdir.glob(pattern) for pattern in pattern_2d ]))
        except StopIteration:
            raise RuntimeError(f"GMAO 2D file not found from patterns: {pattern_2d} at directory {gmao_fdir}")
        ...
        with ifile_hlp.open_h5(fname_3d) as f:
            for v in ["QV", "T", "lat", "lev", "lon"]:
                ...
                if res[v].shape[0] == 1:
                    res[v] = res[v][0]
        with ifile_hlp.open_h5(fname_2d) as f:
