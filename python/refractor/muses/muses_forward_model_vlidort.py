...
class MusesTropomiForwardModelVlidort(MusesForwardModelVlidortBase):
    ...
    def tropomi_fm(self, i_uip, i_osp_dir=None, i_obs=None, skip_raman_copy=False):
...
class MusesOmiForwardModelVlidort(MusesForwardModelVlidortBase):
    ...
    def omi_fm(self, i_uip, i_osp_dir=None, i_obs=None, skip_raman_copy=False):
        # Temp, we'll pull some of this over and get other parts into mpy
        from refractor.muses_py import (
            pack_omi_jacobian,
            rev_and_fm_map,
        ...
        # Create atmospheric jacobians
        jacobians_atm_ils = None  # Start out with None.  It will be set to a list of dictionary if i_uip['num_atm_k'] is greater than 0.
        if i_uip["num_atm_k"] > 0:
            k_temp = []
        ...
        # Defined jacobian arrays for the following "species":
        # [OMISURFACEALBEDOUV1 OMISURFACEALBEDOUV2 OMISURFACEALBEDOUV2 OMICLOUDFRACTION OMINRADWAVUV1 OMINRADWAVUV2 OMIRINGSFUV1 OMIRINGSFUV2]
        ...
        # These next arrays are not used anywhere in IDL so they are commented out.
        # jacobian_OMICLOUDFRACTION = np.zeros(shape=(nfreq_tot),dtype=np.float64)
        # jacobian_OMINRADWAVUV1 = np.zeros(shape=(nfreq_tot),dtype=np.float64)
        # jacobian_OMINRADWAVUV2 = np.zeros(shape=(nfreq_tot),dtype=np.float64)
        # jacobian_OMIRINGSFUV1 = np.zeros(shape=(nfreq_tot),dtype=np.float64)
        # jacobian_OMIRINGSFUV2 = np.zeros(shape=(nfreq_tot),dtype=np.float64)

        # AT_LINE 160 OMI/omi_fm.pro
        # prepare model ATM input

        # ALLOCATE MEMORY FOR ATMOSPHERIC JACOBIANS for the clear sky cases
        nlayers = atmparams["nlayers"]
        atm_clear_jacobians_ils = None
        ...
        if i_uip["num_atm_k"] > 0:
            ...
            atm_clear_jacobians_ils = []  # Start out with an empty list.  We will use append() to add each structure.
            for ii in range(0, i_uip["num_atm_k"]):
                atm_clear_jacobians_ils.append(
                    copy.deepcopy(k_structure)
                )  # Make a deepcopy so each element will have its own memory.
            # After the above loop, we now have the correct number of k_structure in atm_clear_jacobians_ils.

            for ii in range(0, len(jacob_str)):
                if jacob_str[ii] in ("O3", "SO2", "NO2"):
                    atm_clear_jacobians_ils[cnt]["species"] = jacob_str[
                        ii
                    ]  # Set the 'species' field in atm_clear_jacobians_ils[cnt] element.
                    cnt = cnt + 1
                # end if (len(uu) > 0):
            # end for ii in range(0,len(jacob_str)):
        # end if (i_uip['num_atm_k'] > 0):

        # AT_LINE 176 OMI/omi_fm.pro
        ...
        if i_uip["num_atm_k"] > 0:
            ...
            atm_cloud_jacobians_ils = []  # replicate(temporary(k_structure),uip.num_atm_k)
            for ii in range(0, i_uip["num_atm_k"]):
                atm_cloud_jacobians_ils.append(
                    copy.deepcopy(k_structure)
                )  # Make a deepcopy so each element will have its own memory.
            # After the above loop, we now have the correct number of k_structure in atm_cloud_jacobians_ils.

            for ii in range(0, len(jacob_str)):
                if jacob_str[ii] in ("O3", "SO2", "NO2"):
                    atm_cloud_jacobians_ils[cnt]["species"] = jacob_str[
                        ii
                    ]  # Replace the 'thisisadummystring' with the actual name of the species
                    cnt = cnt + 1
                # end if (len(uu) > 0:
            # end for ii in range(0,len(jacob_str)):
        # end if (i_uip['num_atm_k'] > 0):
        ...
        for ii_mw in range(0, mw_account["mw_cnt"]):
            ...
            v_ils_mw = mw_account[
                ...
                mws : mwf + 1
            ]  # Remember that Python slices does not include the end point so we have to add 1 to match IDL.
        ...
        # AT_LINE 248 OMI/omi_fm.pro

        # Pack Radiance and Jacobians
        # Note that the returned value of o_jacobian_pack from pack_omi_jacobian() can be None if the value of num_par is 0,
        # so becareful accessing o_jacobian_pack.
        # print(function_name,"#0001:i_uip['jacobians']",len(i_uip['jacobians']),i_uip['jacobians'])
