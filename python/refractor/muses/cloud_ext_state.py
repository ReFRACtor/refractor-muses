...
class CloudExtState(rf.GenericStateImpBase):
    ...
    def __init__(self, cloud_extv : np.ndarray, cloud_ext_sd: rf.SpectralDomain,
        ...
        if cloud_extv.shape != cloud_ext_sd.data.shape:
            raise RuntimeError("cloudext and cloudext spectral domain need to be the same size")
    ...
    def clone(self) -> rf.GenericState:
        return CloudExtState(self.cloud_ext, self.cloud_ext_spectral_domain, self.state_mapping)
