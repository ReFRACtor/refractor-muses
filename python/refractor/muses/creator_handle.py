...
class CreatorHandleSet(PriorityHandleSet):
    '''We have a few objects that we want to be able to create to support
    a retrieval, e.g, a ForwardModel for a particular instrument.

    This is the primary place where we extend the retrieval strategy, e.g.,
    have a new ForwardModel for a new instrument.

    We use a CreatorHandleSet, which is a specialization of a PriorityHandleSet.
    New functionality can be added by adding new handles to create the new objects,
    this plugs into the existing RetrievalStategy for doing a retrieval.
    ...
    In practice you create a simple CreatorHandle class that just creates the
    something like a ForwardModel or Observation, and register with the
    CreatorHandleSet, e.g., ForwardModelHandleSet or ObservationHandleSet.
    Take a look at the existing examples (e.g. the unit tests) - the design
    seems complicated but is actually pretty simple to use in pratice.

    Note a CreatorHandle can assume that it called for the same target, until
    ...
    one call to the next (e.g., read a L1B file once).
    ...
    def __init__(self, creator_func_name : str):
        '''Constructor, takes the name of the creator function (e.g., "observation").'''
        super().__init__()
...
class CreatorHandle:
    '''Base class for handles used by CreatorHandleSet. Note we use duck typing,
    so you don't actually need to derive from this object. But it can be
    ...
    # This should either return an object if we can create it, or None if we can't
    # (and the CreatorHandleSet goes on to try the next handle.
