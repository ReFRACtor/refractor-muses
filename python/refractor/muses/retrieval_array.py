...
def _make_maps(
    ...
    if log_interp:
        x = np.log(x).view(FullGridMappedArray)
    if x[1] < x[0]:
        x = -x
    n = len(x)
    ind = np.array([t for t in ind if t <= n])
    m = len(ind)
    m_pars = np.zeros(shape=(n, m), dtype=np.float64)
    m_state = np.zeros(shape=(m, n), dtype=np.float64)

    ret_x = x[ind - 1]
    for jj in range(len(ret_x) - 1):
        ind1 = np.where(x >= ret_x[jj])[0]
        ind2 = np.where(x <= ret_x[jj + 1])[0]
        for kk in range(min(ind1), max(ind2) + 1):
            x1 = ret_x[jj]
            x2 = ret_x[jj + 1]
            if x1 == x2:
                raise RuntimeError("Check retrieval lvls for duplicates ")
            coef = np.float32(x2 - x[kk]) / np.float32(x2 - x1)
            m_state[jj, kk] = coef
            m_state[jj + 1, kk] = 1.0 - coef

    m_pars[:, :] = m_state.T @ np.linalg.inv(m_state @ m_state.T)

    return m_state, m_pars
...
def _from_x_subset_exclude_gap(
    ...
    for k in range(m_state.shape[1]):
