...
class OspCovarianceMatrix:
    ...
    def __init__(self, covariance_directory: Path) -> None:
        ...
        for fname in covariance_directory.glob("Covariance_Matrix_*.asc"):
            m = re.match(r'Covariance_Matrix_(\w+)_(\w+)_(\d+)([SN])_(\d+)([SN]).asc', fname.name)
            if(m):
                sid = m[1]
                maptype = m[2].lower()
    ...
    def read_cov(self, sid : StateElementIdentifier, map_type: str, latitude: float) -> np.ndarray:
        ...
        d = np.array(TesFile(self.filename_data[str(sid)][map_type.lower()][latitude]).table)[:,3:].astype(float)
        if(d.shape != (1,1)):
