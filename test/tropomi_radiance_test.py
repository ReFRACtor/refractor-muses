...
def test_tropomi_radiance(tropomi_uip_step_2):
    ...
    rf_uip.uip_tropomi['jacobians'] = np.append(rf_uip.uip_tropomi['jacobians'], ("TROPOMIRADSQUEEZEBAND3"))
    mrad = TropomiRadiancePyRetrieve(rf_uip)
    sv = rf.StateVector()
    sv.add_observer(mrad)
    x = [rf_uip.tropomi_params["solarshift_BAND3"],
    ...
    mrad2 = TropomiRadianceRefractor(rf_uip,["BAND3",], fname)
    sv2 = rf.StateVector()
    ...
    npt.assert_allclose(r.spectral_range.uncertainty,
                        r2.spectral_range.uncertainty, rtol=2e-5)
...
def test_bad_sample_tropomi_radiance(tropomi_uip_step_2):
    ...
    rf_uip = tropomi_uip_step_2
    rdata = pickle.load(open(rf_uip.run_dir + "/Input/Radiance_TROPOMI_.pkl",
                             "rb"))
    rdata['Earth_Radiance']['EarthRadianceNESR'][1::15] = -999
    pickle.dump(rdata, open(rf_uip.run_dir + "/Input/Radiance_TROPOMI_.pkl",
                            "wb"))
    mrad = TropomiRadiancePyRetrieve(rf_uip)
    fname = glob.glob(f"{rf_uip.run_dir}/Input/Radiance_TROPOMI*.pkl")[0]
    mrad2 = TropomiRadianceRefractor(rf_uip,["BAND3",],fname)
    ...
    npt.assert_allclose(mrad.spectral_domain(0,inc_bad_sample=True).data,
