...
def test_measurement_id(isolated_dir, osp_dir, gmao_dir):
    ...
    assert (
        mid["omi_calibrationFilename"]
        == f"{osp_dir}/OMI/OMI_Rad_Cal/JPL_OMI_RadCaL_2006.h5"
    )
...
def test_create_muses_airs_observation(isolated_dir, osp_dir, gmao_dir, vlidort_cli):
    ...
    measurement_id = MeasurementIdFile(
        f"{r.run_dir}/Measurement_ID.asc", rconfig, filter_list_dict
    )
    # This is the microwindows file for step 8, determined by just running the full
    # retrieval and noting the file used
    mwfile = f"{osp_dir}/Strategy_Tables/ops/OSP-OMI-AIRS-v10/MWDefinitions/Windows_Nadir_H2O_O3_joint.asc"
...
def test_create_muses_tes_observation(isolated_dir, osp_dir, gmao_dir, vlidort_cli):
    ...
    fmeta = FileFilterMetadata(
        f"{osp_dir}/Strategy_Tables/ssund/Defaults/Default_Spectral_Windows_Definition_File_Filters.asc"
    )
...
def test_create_muses_tropomi_observation(isolated_dir, osp_dir, gmao_dir, vlidort_cli):
    # Don't need a lot from run dir, but this modifies the path in Measurement_ID.asc
    # to point to our test data rather than original location of these files, so go
    # ahead and set this up
...
def test_create_muses_cris_observation(isolated_dir, osp_dir, gmao_dir, vlidort_cli):
    # Don't need a lot from run dir, but this modifies the path in Measurement_ID.asc
    # to point to our test data rather than original location of these files, so go
    # ahead and set this up
    r = MusesRunDir(joint_tropomi_test_in_dir, osp_dir, gmao_dir)
...
def test_create_muses_omi_observation(isolated_dir, osp_dir, gmao_dir, vlidort_cli):
    # Don't need a lot from run dir, but this modifies the path in Measurement_ID.asc
    # to point to our test data rather than original location of these files, so go
    # ahead and set this up
    r = MusesRunDir(joint_omi_test_in_dir, osp_dir, gmao_dir)
