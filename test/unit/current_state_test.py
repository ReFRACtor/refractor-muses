...
def test_current_state_state_info(
    ...
    for i in range(rs.number_retrieval_step):
        ...
        if rs.retrieval_info.n_speciesSys > 0:
            ...
            rf_uipsys = rstepsys.uip_func(
                rs, do_systematic=True, jacobian_speciesIn=None
            )
        ...
        if rs.retrieval_info.n_speciesSys > 0:
            cssys = CurrentStateStateInfo(
                rs.state_info, rs.retrieval_info, do_systematic=True
            )
        else:
            cssys = None
    ...
    cs = CurrentStateStateInfo(
        rs.state_info, rs.retrieval_info, retrieval_state_element_override=["H2O"]
    )
    assert cs.fm_state_vector_size == csuip.fm_state_vector_size
    assert cs.fm_sv_loc == csuip.fm_sv_loc
