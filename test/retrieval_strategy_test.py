...
def test_original_retrieval_cris_tropomi(osp_dir, gmao_dir, vlidort_cli):
    ...
    r = MusesRunDir(
        ...
        osp_dir,
        gmao_dir,
        path_prefix="original_retrieval_cris_tropomi",
    )
...
def test_retrieval_strategy_cris_tropomi(
    osp_dir, gmao_dir, vlidort_cli, python_fp_logger
):
    ...
    r = MusesRunDir(
        ...
        osp_dir,
        gmao_dir,
        path_prefix="retrieval_strategy_cris_tropomi",
    )
    rs = RetrievalStrategy(
        f"{r.run_dir}/Table.asc",
        writeOutput=True,
        writePlots=True,
    ...
    diff_is_error = True
    compare_run(
        compare_dir, "retrieval_strategy_cris_tropomi", diff_is_error=diff_is_error
    )
...
def test_original_retrieval_airs_omi(osp_dir, gmao_dir, vlidort_cli):
    ...
    r = MusesRunDir(
        ...
        osp_dir,
        gmao_dir,
        path_prefix="original_retrieval_airs_omi",
    )
    # rmi = RefractorMusesIntegration(vlidort_cli=vlidort_cli, save_debug_data=True)
    # rmi.register_with_muses_py()
...
@long_test
def test_retrieval_strategy_airs_omi(osp_dir, gmao_dir, vlidort_cli, python_fp_logger):
    """Full run, that we then compare the output files to expected results.
    This is not really a unit test, but for convenience we have it here.
    Note that a "failure" in the comparison might not actually indicate a problem, just
    that the output changed. You may need to look into detail and decide that the
    ...
    r = MusesRunDir(
        ...
        osp_dir,
        gmao_dir,
        path_prefix="retrieval_strategy_airs_omi",
    )
    rs = RetrievalStrategy(
        f"{r.run_dir}/Table.asc",
        vlidort_cli=vlidort_cli,
    ...
    diff_is_error = True
    compare_run(compare_dir, "retrieval_strategy_airs_omi", diff_is_error=diff_is_error)
...
def test_run_fabiano_refractor(isolated_dir, osp_dir, gmao_dir, vlidort_cli):
    rs, kwargs = RetrievalStrategy.load_retrieval_strategy(
        "/home/smyth/muses/refractor-muses/compare_fabiano_refractor/20200701_204_05_29_0/retrieval_step_10.pkl",
        osp_dir=osp_dir,
        gmao_dir=gmao_dir,
        vlidort_cli=vlidort_cli,
        change_to_dir=True,
...
def test_run_fabiano_vlidort(isolated_dir, osp_dir, gmao_dir, vlidort_cli):
    print(vlidort_cli)
    rs, kwargs = RetrievalStrategy.load_retrieval_strategy(
        "/home/smyth/muses/refractor-muses/compare_fabiano_refractor_vlidort/20200701_204_05_29_0/retrieval_step_10.pkl",
        osp_dir=osp_dir,
        gmao_dir=gmao_dir,
        vlidort_cli=vlidort_cli,
        change_to_dir=True,
...
def test_original_retrieval_tes(osp_dir, gmao_dir, vlidort_cli):
    ...
    r = MusesRunDir(
        f"{test_base_path}/tes/in/sounding_1",
        osp_dir,
        gmao_dir,
        path_prefix="original_retrieval_tes",
    )
    r.run_retrieval(vlidort_cli=vlidort_cli)


@long_test
def test_retrieval_strategy_tes(osp_dir, gmao_dir, vlidort_cli, python_fp_logger):
    """Full run, that we then compare the output files to expected results.
    This is not really a unit test, but for convenience we have it here.
    Note that a "failure" in the comparison might not actually indicate a problem, just
    that the output changed. You may need to look into detail and decide that the
    ...
    r = MusesRunDir(
        tes_test_in_dir, osp_dir, gmao_dir, path_prefix="retrieval_strategy_tes"
    )
    rs = RetrievalStrategy(
        f"{r.run_dir}/Table.asc",
        vlidort_cli=vlidort_cli,
    ...
    diff_is_error = True
    compare_run(compare_dir, "retrieval_strategy_tes", diff_is_error=diff_is_error)
...
@skip
def test_failed_tes(osp_dir, gmao_dir, vlidort_cli, python_fp_logger):
...
def test_original_retrieval_oco2(osp_dir, gmao_dir, vlidort_cli):
    ...
    r = MusesRunDir(
        f"{test_base_path}/oco2/in/sounding_1",
        osp_dir,
        gmao_dir,
        path_prefix="original_retrieval_oco2",
    )
    r.run_retrieval(vlidort_cli=vlidort_cli)
...
def test_original_retrieval_airs_irk(osp_dir, gmao_dir, vlidort_cli):
    ...
    r = MusesRunDir(
        f"{test_base_path}/airs_omi/in/sounding_1_irk",
        osp_dir,
        gmao_dir,
        path_prefix="original_retrieval_airs_irk",
    )
    r.run_retrieval(vlidort_cli=vlidort_cli)


@long_test
def test_retrieval_strategy_airs_irk(osp_dir, gmao_dir, vlidort_cli, python_fp_logger):
    """Full run, that we can compare the output files. This is not
    ...
    r = MusesRunDir(
        ...
        osp_dir,
        gmao_dir,
        path_prefix="retrieval_strategy_airs_irk",
    )
    ...
    diff_is_error = True
    compare_run(compare_dir, "retrieval_strategy_airs_irk", diff_is_error=diff_is_error)
...
@long_test
def test_only_airs_irk(osp_dir, gmao_dir, vlidort_cli, python_fp_logger):
    ...
    r = MusesRunDir(
        airs_irk_test_in_dir, osp_dir, gmao_dir, path_prefix="only_airs_irk"
    )
    rs = RetrievalStrategy(f"{r.run_dir}/Table.asc", vlidort_cli=vlidort_cli)
    ihandle = OmiForwardModelHandle(
        use_pca=True,
    ...
    diff_is_error = True
    compare_dir = airs_irk_test_expected_dir
    compare_irk(compare_dir, "only_airs_irk", diff_is_error=diff_is_error)
