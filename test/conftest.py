...
def pytest_addoption(parser):
    parser.addoption("--run-long", action="store_true",
                     help="run long tests")
    parser.addoption("--skip-old-py-retrieve-test", action="store_true",
...
def pytest_collection_modifyitems(config, items):
    ...
    if config.getoption("--skip-old-py-retrieve-test"):
        skip_old_py_retrieve_test = pytest.mark.skip(reason="--skip-old-py-retrieve-test option skips this test")
        for item in items:
