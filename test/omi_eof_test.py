...
def test_eof_omi(osp_dir, gmao_dir, vlidort_cli):
    ...
    r = MusesRunDir(omi_test_in_dir, osp_dir, gmao_dir, path_prefix="omi_eof")
    ...
    subprocess.run(
        f'sed -i -e "s/OMINRADWAVUV1/OMINRADWAVUV1,OMIEOFUV1,OMIEOFUV2/" {r.run_dir}/Table.asc',
        shell=True,
    )
    ...
    if True:
        ...
        subprocess.run(
            "h5dump -d OMI_EOF_UV1 -A 0 omi_eof/20160414_23_394_11_23/Products/Products_L2-O3-0.nc",
            shell=True,
        )
        subprocess.run(
            "h5dump -d OMI_EOF_UV2 -A 0 omi_eof/20160414_23_394_11_23/Products/Products_L2-O3-0.nc",
            shell=True,
        )
...
def test_eof_airs_omi(osp_dir, gmao_dir, vlidort_cli):
    ...
    if True:
        # Print out output of EOF, just so we have something to see
        subprocess.run(
            "h5dump -d OMI_EOF_UV1 -A 0 airs_omi_eof/20160414_23_394_11_23/Products/Products_L2-O3-0.nc",
            shell=True,
        )
        subprocess.run(
            "h5dump -d OMI_EOF_UV2 -A 0 airs_omi_eof/20160414_23_394_11_23/Products/Products_L2-O3-0.nc",
            shell=True,
        )
