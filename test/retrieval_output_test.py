...
def joint_tropomi_output(isolated_dir):
    ...
    rs, rstep, kwargs = set_up_run_to_location(
        joint_tropomi_test_in_dir, 12, "retrieval step"
    )
    yield rs, rstep, kwargs
    compare_dir = joint_tropomi_test_refractor_expected_dir
    diff_is_error = True
    compare_run(compare_dir, "20190807_065_04_08_5", diff_is_error=diff_is_error)


@pytest.mark.skipif(
    not run_retrieval_output_test,
...
def test_retrieval_pickle_results(joint_tropomi_output):
    ...
    assert os.path.exists(
        f"{rs.run_dir}/Step12_H2O,O3,EMIS_TROPOMI/Diagnostics/results.pkl"
    )
...
def test_retrieval_plot_radiance(joint_tropomi_output):
    ...
    for fname in (
        ...
        assert os.path.exists(
            f"{rs.run_dir}/Step12_H2O,O3,EMIS_TROPOMI/StepAnalysis/{fname}"
        )
...
def test_retrieval_irk_output(isolated_dir):
    rs, rstep, kwargs = set_up_run_to_location(airs_irk_test_in_dir, 6, "IRK step")
    ...
    diff_is_error = True
    compare_run(compare_dir, "20160401_231_049_29", diff_is_error=diff_is_error)
